...
from src.depends import get_current_user, get_db
...
async def read_one_item(item_id: int,
    ...
    verify = crud.verify_item(db, current_user.id, item_id)
    if verify == False:
...
@router.post("/edit/{id}", response_model=schemas.Item)
...
@router.get("/items/recycle", response_model=list[schemas.Item])
